<?
	...
	$arrListaCampos = array
						(
							...
							'ChequesComercios.dFechaRegistro', 
							'ChequesComercios.fImporte'											
						 );
	...
	$Tabla = 'ChequesComercios'; // Tabla sobre la cual trabajaremos
	$PK = 'id';	// Nombre del campo Clave Principal
	$CampoOrdenPre = 'ChequesComercios.dFechaEmision'; // Campo de orden predeterminado
	...
	{					
		...
		if($_POST['dFechaDesde'] != "" && $datos['dFechaDesde'] != "__/__/____"){			
			$conditions[] = "ChequesComercios.dFechaEmision >= '".dateToMySql($_POST['dFechaDesde'])."'";
		}
		
		if($_POST['dFechaHasta'] != "" && $_POST['dFechaDesde'] != "__/__/____"){
			$conditions[] = "ChequesComercios.dFechaEmision <= '".dateToMySql($_POST['dFechaHasta'])."'";
		}		
	...
	else
	{
		...
		if($fecha_desde != "" && $fecha_desde != "__/__/____"){			
			$conditions[] = "ChequesComercios.dFechaEmision >= '".dateToMySql($fecha_desde)."'";
		}
		
		if($fecha_hasta != "" && $fecha_hasta != "__/__/____"){
			$conditions[] = "ChequesComercios.dFechaEmision <= '".dateToMySql($fecha_hasta)."'";
		}
	...
	$sqlDatos="Call usp_getValoresDePago(\"$sCondiciones\");";
	$sqlDatos_sLim="Call usp_getValoresDePago(\"$sCondiciones_sLim\");";
  ...
  $sCadena .= " en la base de datos.</p>
	...
	{			
		...
		<tr id="Liquidacion<?php echo $PK;?>">
			...
			<td width="5%" align="left" <?php echo $sClase;?>>&nbsp;<?=$rs['sRazonSocial'];?></td>
			<td width="5%" align="left" <?php echo $sClase;?>>&nbsp;<?=$rs['dFechaRegistro'];?></td>
