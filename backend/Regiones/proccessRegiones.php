...
if ( stristr($_SERVER["HTTP_ACCEPT"],"application/xhtml+xml") ) {
	...
	if(isset($_GET["!nativeeditor_status"]) && $_GET["!nativeeditor_status"] == 'inserted'){
			...
			$toauditory = "insercion de Region ::: {$_GET['sNombre']}, '{$_GET['sNumero']}'";
			
			$iduser = $_SESSION['id_user'];

			$newId = $oMysql->consultaSel("CALL usp_InsertTable(\"Regiones\",\"$set\",\"$values\",\"{$iduser}\",\"0\",\"$toauditory\");");
			...
			$values = "Regiones.id='{$_GET['gr_id']}'";

			$set = "Regiones.sEstado='B'";

			$toauditory = "baja de Region ::: id = {$_GET['gr_id']}";

			$iduser = $_SESSION['id_user'];

			$newId = $oMysql->consultaSel("CALL usp_UpdateTable(\"Regiones\",\"$set\",\"$values\",\"{$iduser}\",\"0\",\"$toauditory\");");
