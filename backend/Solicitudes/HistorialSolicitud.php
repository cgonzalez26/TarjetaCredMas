...
$result = $oMysql->consultaSel("SELECT EstadosSolicitudes.idEmpleado,EstadosSolicitudes.idTipoEstadoSolicitud,TiposEstadosSolicitudes.sNombre as 'sEstado', IFNULL(DATE_FORMAT(EstadosSolicitudes.dFechaRegistro,'%d/%m/%Y  %H:%i'),'') as 'dFechaRegistro',EstadosSolicitudes.sMotivo 
						FROM EstadosSolicitudes
						LEFT JOIN TiposEstadosSolicitudes ON TiposEstadosSolicitudes.id = EstadosSolicitudes.idTipoEstadoSolicitud
						WHERE EstadosSolicitudes.idSolicitud={$_GET['id']} ORDER BY dFechaRegistro ASC");
...
<center>
	...
	<tr>
		<td align="center" style="height:25px"><b>Historial de Solicitudes</b></td>
	</tr>
	<tr>
		<td align="left" style="height:25px"><span style="">Nro. de Solicitud :&nbsp;<? echo $_GET['sNumero']; ?></span></td>
	</tr>
	</table>
