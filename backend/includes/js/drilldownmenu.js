...
function drilldownmenu(setting){
	this.sublevelarrow={src:'../includes/images/right.gif', width:'8px', top:'3px', left:'6px'} //full URL to image used to indicate there's a sub level
	this.breadcrumbarrow='../includes/images/right.gif' //full URL to image separating breadcrumb links (if it's defined)
	this.loadingimage='../includes/images/loader.gif' //full URL to 'loading' image, if menu is loaded via Ajax
	this.homecrumbtext='Inicio' //Top level crumb text
	this.titlelength=35 //length of parent LI text to extract from to use as crumb titles
	this.backarrow='../includes/images/leftarrow.gif' //full URL to image added in front of back LI 
...
drilldownmenu.prototype.buildmenu=function($){
	...
	this.$uls.each(function(i){ //loop through each UL
		...
		if (i==0){ //if topmost UL
			$('<li class="backcontroltitle">Navegacion</li>').prependTo($thisul).click(function(e){e.preventDefault()})
			...
			$('<li class="backcontrol"><img src="'+thisdrill.backarrow+'" style="border-width:0" /> Volver </li>') //add back LI item
			...
			/*var $arrowimg=$('<img class="arrowclass" />').attr('src', thisdrill.sublevelarrow.src).css({position:'absolute', borderWidth:0, paddingTop:thisdrill.sublevelarrow.top}).prependTo($anchorlink)*/
...
drilldownmenu.prototype.slidemenu=function(order, disableanimate){
	...
	if ($targetul.data('specs').h > this.$maindiv.data('h')){
		...
		this.$arrowimgs.css('left', 227) //adjust arrow position if scrollbar is added
