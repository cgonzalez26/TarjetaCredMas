...
<script>
	...
	var flds = "Codigo,Descripcion,Porcentaje Retencion,Importe Minimo Ret,Importe Minimo Excluido,Estado";
	
	_mygrid.setHeader(flds);
	
	/*_mygrid.attachHeader("#text_filter,#text_filter");*/
	
	_mygrid.setInitWidths("100,100,100,100,100,100");
	
	_mygrid.setColAlign("left,left,left,left,left,left");
	
	_mygrid.setColTypes("ro,txt,ed,ed,ed,coro");
    
	_mygrid.setColumnIds("sCodigo,sDescripcion,fPorcentajeRetencion,fImporteMinimoRetencion,fImporteMinimoExcluido,sEstadoRetencion");
	...
	combo = _mygrid.getCombo(5);
	combo.put("A","A");
	combo.put("B","B");

	_mygrid.init();

   	_mygrid.loadXML("xmlRetencionesGanancias.php");
   	

	myDataProcessor = new dataProcessor("processRetencionesGanancias.php");
