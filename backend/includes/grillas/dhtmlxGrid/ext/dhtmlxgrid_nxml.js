...
dhtmlXGridObject.prototype.printViewLex = function(sEncabezado,sTitulo,iCantReg,iFolio_){
	   //var html="<style>TD { font-family:Arial; text-align:center;} </style>";
	   
	   html="<base  href='"+document.location.href+"'></base>";
	   //html="<html><body>";
       html+= '<table width="100%" border="1px"  rules="all" cellpadding="0" cellspacing="0" style="font-family:Courier New,Dot Matrix,Times New Roman;font-size:11pt;">';
       var row_length = this.rowsCol.length;
       var col_length = this._cCount;
       var width = this._printWidth();
       var iFolio=iFolio_;   
      
      html +="<tr><th style='border:0px;' width='100%' colspan='"+col_length+"'> "+sEncabezado+"</th></tr> "
      // FILAS= "+(row_length + 1)+
      html +="<tr><th style='border:0px;' colspan='"+col_length+"'> &nbsp;"+sTitulo+"&nbsp; </th></tr> "
      ...
      html += '<tr>';
        for (var i=0; i<col_length; i++){
         ...
         for (var j=1; j<colspan; j++)
         	width[i]+=width[j];
    	html += '<td rowspan="'+rowspan+'" width="'+width[i]+'%" style="padding-left:2px;padding-right:2px;background-color:lightgrey !important;font-weight:bold !important;" colspan="'+colspan+'">'+this.getHeaderCol(i)+'</td>';
         i+=colspan-1;
        }
      html += '</tr>';
      
      sCadenaEncabezado=html;
      
      //html = html.replace('{FOLIO}',iFolio);
       ...
       for (var i=0; i<row_length; i++) {
       	 iCont++; 
       	 if(iCont > iCantReg){
       	 	html += "</table><div  id='saltopagina' style='display:block;page-break-before:always;' /></div>";
       	 	html += sCadenaEncabezado;
       	 	iCont=0;
       	 	iFolio++;
       	 	//html = html.replace('{FOLIO}',iFolio);
       	 }else{ 
	           ...
	           for (var j=0; j<col_length; j++) {
	            ...
	            html += '<td style="font-size:8pt;border:1px;padding-left:2px;padding-right:2px;'+color+align+'">'+(value===""?"&nbsp;":value)+'</td>';
	           }
	         html += '</tr>';
       	 }
      ...
      html += '</table>';
      //html += '</body></html>';
      document.getElementById('impresiones').innerHTML=html;
      //window.print();
      var d = window.open('', '_blank');
      d.document.write(html);
      d.window.print();
