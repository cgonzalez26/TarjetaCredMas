...
function eXcell_sub_row(cell){
	...
	this.getTitle = function(){ return "click to expand|collase"; }
}
...
dhtmlXGridObject.prototype._expandMonolite=function(n,show,hide){
	...
	if (row._expanded && !show){
		...
		if (that._fake)
			that._fake.rowsAr[row.idd].style.height="20px";
		...
		if (!that._flow) {
			...
			that.attachEvent("onEditCell",function(a,b,c){  if ((a!==2) && this._flow[b]) this._expandMonolite.apply(this._flow[b].ctrl.firstChild,[0,false,true]);  return true; });
...
dhtmlXGridObject.prototype._sub_row_render={
	...
	"grid":function(that,d,td,c){
		   ...
		   td._sub_grid.loadXML(c,function(){
				...
				td._sub_grid.enableAutoHeight(false);
				that._correctMonolite();
				td._sub_row_type=null;
		   	});		   
	}
...
dhtmlXGridObject.prototype._detectHeight=function(d,td,h){
	var l=td.offsetLeft+td.offsetWidth;
		...
		var h=h||d.scrollHeight+4;
		d.style.overflow="hidden";
		d.style.height=h+"px";		
		td.parentNode.style.height=20+h*1+"px";	
		if (_isKHTML)
			td.style.height=20+h*1+"px";	
		if (this._fake){
			var tr=this._fake.rowsAr[td.parentNode.idd];
			tr.style.height=20+h*1+"px";	
		}
}
dhtmlXGridObject.prototype._correctMonolite=function(mode){
	for (var a in this._flow)
		if (this._flow[a] && this._flow[a].tagName=="DIV")
			if (this.rowsAr[a]){			
				this._flow[a].style.top=this.rowsAr[a].offsetTop+20+"px";
				if (mode) {
