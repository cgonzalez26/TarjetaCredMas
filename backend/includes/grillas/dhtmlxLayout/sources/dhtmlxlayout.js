...
function dhtmlXLayoutObject(base, view, skin) {
	...
	if (!base._skipChecksOnStartUp) {
		...
		if (base._isLayoutCell == true) {
			...
			base.window._isLayoutCell = true;
			base = base.window;
		}
		// check if window
		if (base._isWindow == true) {
			var layout = base.attachLayout(view, this.skin);
			if (!base._isLayoutCell) { layout._inWinInit = true; }
			if (this._updateDimensions == true) { layout._dimension = new Array(0, 0); }
			return layout;
		} else if (base == document.body) {
			// document.body.style.overflow = "hidden";
			//.className += " dhxLayoutFullScreenedInit";
			if (document.getElementsByTagName("html")[0] != null) document.getElementsByTagName("html")[0].className += " dhxLayoutFullScreenedInit";
			document.body.className += " dhxLayoutFullScreenedInit";
				...
				if (!td._isSep) {
					...
					*/
					td.attachStatusBar = function() {
						...
						that._updateSelfContent(this.window);
						return this._status;
					}
					/**
	...
	*/
	...
	this._updateSelfContent = function(win) {
		// change window's content height
		// use with docktype?
		return;
		if (!win._isDocked) return;
		var obj = this.polyObj[String(win.getId()).replace(/^w/,"")].childNodes[1];
		obj.childNodes[2].style.height = parseInt(obj.style.height)-(win.toolbar!=null?win._toolbarH:0)-(win.menu!=null?win._menuH:0)-(win.sb!=null?win._sbH:0)-2+"px";
	}
	...
	this._defineWindowMinDimension = function(win, inLayout) {
		if (inLayout == true) {
			...
			if (!win._isFullScreened) {
				...
				var dim = new Array(this.base.offsetWidth, this.base.offsetHeight);
			}
		}
		...
		if (inLayout == true) {
			...
			return new Array("", minWidth, minHeight); // needed for setSizes in fullscreened init
		}
	}
	...
	*/
	this.setSizes = function(winObj) {
		...
		// var minDim;// = new Array("", 0, 0);
		
		// window init
		if (this._inWinInit && !minDim) {
			//console.log("win")
			var minDim = this._defineWindowMinDimension(winObj);
		}
		
		// fullscreened init
		if (winObj._isFullScreened && !minDim) {
			//console.log("fullscreen")
			var minDim = this._defineWindowMinDimension(winObj);
		}
		
		// layiyt's cell init
		if (this._parentWindow._isLayoutCell && !minDim) {
			//console.log("cell")
			var minDim = new Array("", 0, 0);
		}
		...
		if (!minDim) {
			//console.log("none")
			//var minDim = new Array("", 65536, 65536); // broken in case of attached menu
			var minDim = new Array("", 0, 0);
		}
		
		// var minDim = (winObj._isFullScreened?this._defineWindowMinDimension(winObj):(this._parentWindow._isLayoutCell?new Array("", 0, 0):new Array("", 65536, 65536)));
		
		// console.log(minDim)
		
		var bw = this.base.childNodes[0].childNodes[0].offsetWidth;
		var bh = this.base.childNodes[0].childNodes[0].offsetHeight;
		...
		if (ww < minDim[1]) ww = minDim[1];
		if (wh < minDim[2]) wh = minDim[2];
