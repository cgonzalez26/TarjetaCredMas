<?
	...
	$NombreMod = 'Cupones'; // Nombre del modulo
	$NombreTipoRegistro = 'Cupones';	// Nombre tipo de registro
	$NombreTipoRegistroPlu = 'Cupones'; // Nombre tipo de registros en Plural
	...
	$aParametros['FORM'] = "Cupones.php";
	$aParametros['TituloBuscador'] = "Cupones";
...
echo parserTemplate( TEMPLATES_XHTML_DIR . "/Bloques/BloqueBuscadorCupones.tpl",$aParametros);

//if(in_array(AGREGAR,$arrayPermit)){
	?>
	<center>
	<p><img src="../includes/images/Formularios/icoNuevo.gif" align="absmiddle" alt="Nuevo"> <a href="AdminCupones.php?_op=<? echo _encode('new'); ?>"><? if($Masculino) echo 'Nuevo '; else echo 'Nueva '; echo $NombreTipoRegistro;?></a></p>
	<?
//}
...
if ($result){	
  ...
  $sCadena .= "<center><div style='width:80%'><p>Hay ".$CantReg." ";
  if ($CantReg>1) $sCadena .= $NombreTipoRegistroPlu; 
  else $sCadena .= $NombreTipoRegistro;

  $sCadena .= " en la base de datos.</p></div></center>
...
<script>
	...
	function fichaCupon(_i){
		window.location ="AdminCupones.php?_i="+ _i +"&_op=<?php echo _encode('wiew');?>";
	}
	...
	function marcarCuponesObservado(_i,_cupon_){
		...
		_createWindows('MarcarCuponesObservado.php?_i=' + _i + '&_n=' + _cupon_);
	...
	function mostrarDetalles(_i){
		window.location ="AdminCupones.php?_i="+ _i +"&_op=<?php echo _encode('wiew-details');?>";
	}
	
	function mostrarCuotas(_i){
		window.location ="AdminCupones.php?_i="+ _i +"&_op=<?php echo _encode('wiew-only-cuotas');?>";
	}	
