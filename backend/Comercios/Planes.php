<?
	...
	$NombreMod = 'Planes'; // Nombre del modulo
	$NombreTipoRegistro = 'Plan';	// Nombre tipo de registro
	$NombreTipoRegistroPlu = 'Planes'; // Nombre tipo de registros en Plural
	$Masculino = true;	// Genero del tipo de registro (true: masculino - false: femenino)
	$arrListaCampos = array('TiposPlanes.sNombre','Planes.sNombre','Planes.dVigenciDesde', 'Planes.dVigenciaHasta','Planes.iCantidadCuota','Planes.sEstado');
	$arrListaEncabezados = array('Tipo Plan','Plan','Vigencia Desde','Vigencia Hasta','Cuotas','Estado');
	$Tabla = 'Planes'; // Tabla sobre la cual trabajaremos
	$PK = 'id';	// Nombre del campo Clave Principal
	$CampoOrdenPre = 'Planes.sNombre'; // Campo de orden predeterminado
	...
	//if($sNombre){
		...
		$aCond[]=" Planes.idComercio = '$idcomercio' ";
	...
	$sqlDatos_sLim="Call usp_getPlanes(\"$sCondiciones_sLim\");";
	...
	$oXajax->register( XAJAX_FUNCTION ,"eliminarPlanes");
	$oXajax->register( XAJAX_FUNCTION ,"activarPlanes");
...
<center>
<p><img src="../includes/images/Formularios/icoNuevo.gif" align="absmiddle" alt="Nuevo"><a href="AdminPlanes.php?_op=<? echo _encode('new'); ?>&_ic=<? echo $idcomercio; ?>"><? if($Masculino) echo 'Nuevo '; else echo 'Nueva '; echo $NombreTipoRegistro;?></a></p>
...
if ($result){	
  ...
  $sCadena .= " en la base de datos.</p>
	...
	foreach ($result as $rs ){
		...
		if($rs['sEstado'] == "B"){
			$oBtn->addBoton("planes_wiew_{$idplan}","onclick","fichaPlanes('$idplan','$idcomercio')",'search24','Mostrar',true,true);

			$oBtn->addBoton("planes_act_{$idplan}","onclick","activarPlanes('$idplan','{$rs['sNombre']}')",'Actualizar','Activar',true,true);

		}else{

			//$oBtn->addBoton("planes_planes_{$idcomercio}","onclick","mostrarPlanes('$idcomercio')",'tiposplanes24','tipos planes',true,true);

			$oBtn->addBoton("planes_edi_{$idplan}","onclick","editarPlanes('$idplan','$idcomercio')",'editar24','Editar',true,true);

			$oBtn->addBoton("planes_del_{$idplan}","onclick","eliminarPlanes('$idplan','{$rs['sNombre']}')",'delete24','Eliminar',true,true);
		...
		<tr id="row_comercio_<?php echo $PK;?>" class="">
			<!-- row -->
			<td width="150" align="left" <?php echo $sClase;?>>&nbsp;<?=$rs['sNombreTipoPlan'];?></td>
			<td width="150" align="left" <?php echo $sClase;?>>&nbsp;<?=$rs['sNombre'];?></td>
...
<script>
	...
	function editarPlanes(_i,_ic){
		window.location ="AdminPlanes.php?_i="+ _i +"&_op=<?php echo _encode('edit');?>&_ic=" + _ic;
	}

	function eliminarPlanes(_i,_plan_){
		if(confirm("Esta seguro de eliminar el Plan: '"+ _plan_ +"' ?")){
			xajax_eliminarPlanes(_i);
		}
	}
	
	function fichaPlanes(_i,_ic){
		window.location ="AdminPlanes.php?_i="+ _i +"&_op=<?php echo _encode('wiew');?>&_ic=" + _ic;
	}
	
	function activarPlanes(_i,_plan_){
		if(confirm("Esta seguro de activar el Planes: '"+ _plan_ +"' ?")){
			xajax_activarPlanes(_i);
		}
	}
